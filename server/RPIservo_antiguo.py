...
pwm = PCA9685(address=0x40, i2c=i2c)  # Dirección por defecto
pwm.frequency = 50
...
class ServoCtrl(threading.Thread):
    ...
    def moveInit(self):
        self.scMode = 'init'
        for i in range(0,16):
            pwm.channels[i].duty_cycle = self.initPos[i]
    ...
    def initConfig(self, ID, initInput, moveTo):
        if initInput > self.minPos[ID] and initInput < self.maxPos[ID]:
            ...
            if moveTo:
                pwm.channels[ID].duty_cycle = self.initPos[ID]
        else:
    ...
    def moveServoInit(self, ID):
        ...
        for i in range(0,len(ID)):
            pwm.channels[ID[i]].duty_cycle = self.initPos[ID[i]]
        ...
        for i in range(0, self.scSteps):
            for dc in range(0,16):
                if not self.goalUpdate:
                    ...
                    pwm.channels[dc].duty_cycle = self.nowPos[dc]
        ...
        while self.nowPos != self.goalPos:
            for i in range(0,16):
                if self.lastPos[i] < self.goalPos[i]:
                    ...
                    self.nowPos[i] = newNow
                if not self.goalUpdate:
                    pwm.channels[i].duty_cycle = self.nowPos[i]
                if self.ingGoal != self.goalPos:
                    self.posUpdate()
                    return 1
            self.posUpdate()
    ...
    def moveWiggle(self):
        ...
        if self.bufferPos[self.wiggleID] < self.maxPos[self.wiggleID] and self.bufferPos[self.wiggleID] > self.minPos[self.wiggleID]:
            pwm.channels[self.wiggleID].duty_cycle = self.nowPos[self.wiggleID]
        else:
            self.stopWiggle()
        time.sleep(self.scDelay-self.scMoveTime)
    ...
    def moveAngle(self, ID, angleInput):
        ...
        pwm.channels[ID].duty_cycle = self.nowPos[ID]
    ...
    def setPWM(self, ID, PWM_input):
        ...
        pwm.channels[ID].duty_cycle = PWM_input
        self.pause()
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        '''
        '''
        pwm.channels[0].duty_cycle = 560
        time.sleep(1)
        pwm.channels[0].duty_cycle = 100
        time.sleep(2)
        '''
        pass
    pass
